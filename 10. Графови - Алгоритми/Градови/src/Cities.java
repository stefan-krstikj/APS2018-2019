    ...
    public void findRoute(String from, String to) {
        ...
        for (int i = 0; i < num_nodes; i++) {
            if (adjList[i].getInfo().equals(from)) {
                start = adjList[i].getIndex();
            }
            else if(adjList[i].getInfo().equals(to)){
                end = adjList[i].getIndex();
            }
        }

        findRouteHelper(start, end, from);
        System.out.println();
        findRouteHelper(end, start, to);
    }



    void findRouteHelper(int from, int to, String startName) {
...
public class Cities {
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < roads; i++){
            ...
            float weight = Float.parseFloat(in[4]);
