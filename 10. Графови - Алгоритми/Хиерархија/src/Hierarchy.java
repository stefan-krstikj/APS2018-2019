...
class Graph<E> {
    ...
    public int optimal(String start){
        for(int i = 0; i < num_nodes; i++){
            if(adjList[i].getInfo().equals(start))
                return optimalHelper(adjList[i].getIndex());
        }
        return -2;
    }

    int optimalHelper(int start_index) {
        // lista koja kje gi sodrzi MST rebra
        int count = 0;
...
public class Hierarchy {
    public static void main(String[] args) throws Exception{
        ...
        Graph<String> graph = new Graph<>(numberOfEmployees);
        int n = Integer.parseInt(br.readLine());
        for(int i = 0; i < n; i++){
            String[] in = br.readLine().split(" ");
            int x = Integer.parseInt(in[0]);
            int y = Integer.parseInt(in[2]);
            graph.adjList[x].setInfo(in[1]);
            graph.adjList[y].setInfo(in[3]);
            graph.addEdge(x, y, Integer.parseInt(in[4]));
        }
        String pretsedatel = br.readLine();
        br.close();
        System.out.println(graph.optimal(pretsedatel));
    }
}
